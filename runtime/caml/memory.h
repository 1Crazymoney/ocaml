...
#define Alloc_small_aux(result, wosize, tag, profinfo, track) do {     \
  ...
  caml_domain_state* state = Caml_state;                               \
  state->young_ptr -= Whsize_wosize (wosize);                          \
  if (state->young_ptr < state->young_limit) {                         \
    Setup_for_gc;                                                      \
    caml_alloc_small_dispatch((tag), (wosize),                         \
                              (track) | Alloc_small_origin);           \
    Restore_after_gc;                                                  \
  }                                                                    \
  Hd_hp (state->young_ptr) =                                           \
    Make_header_with_profinfo ((wosize), (tag), Caml_black, profinfo); \
  (result) = Val_hp (state->young_ptr);                                \
...
#define CAMLparam0() \
  caml_domain_state *caml__domain_state = Caml_state; \
...
#define CAMLxparam1(x) \
  ...
  CAMLunused_start int caml__dummy_##x = ( \
    (void) caml__frame, \
    (caml__roots_##x.next = caml__domain_state->local_roots), \
    (caml__domain_state->local_roots = &caml__roots_##x), \
...
#define CAMLdrop caml__domain_state->local_roots = caml__frame
